...
void main() {
  group('Studio localization regressions', () {
    test('studio jobs screen no longer has hardcoded studio UX copy', () {
      final source = File('lib/features/studio/presentation/screens/studio_jobs_screen.dart')
          .readAsStringSync();
      ...
      for (final value in forbidden) {
        expect(source.contains(value), isFalse, reason: 'Found hardcoded: $value');
      }
    });

    test('studio public profile screen no longer has hardcoded copy', () {
      final source = File('lib/features/studio/presentation/screens/studio_public_profile_screen.dart')
          .readAsStringSync();
    ...
    test('post job uses sticky bottom submit CTA', () {
      ...
      expect(source, contains('text: l10n.postJobButtonWithRate(_displayRate.toString())'));
    });
  });
}

...
// Run with: bunx convex run testing/dispatchTests:runAllTests
  ...
  handler: async (ctx): Promise<TestResult> => {
    ...
    try {
      ...
      const instructorId = await ctx.runMutation(
        internal.testing.dispatchTests.createTestInstructor,
        {
      ...
      const found = matches.some(
        (m: { instructorId: Id<"users"> }) => m.instructorId === instructorId,
      );
...
export const testZoneModeNotInGeo = internalAction({
  ...
  handler: async (ctx): Promise<TestResult> => {
    ...
    try {
      // Create a zone first
      const zoneId = await ctx.runMutation(
        internal.testing.dispatchTests.createTestZone,
        {
          name: "Test Zone 1",
        },
      );
      ...
      return {
        ...
        error: notFound
          ? undefined
          : "Zone mode instructor incorrectly found in geo index",
      };
    } catch (e) {
      return {
        name: testName,
...
export const testZoneSubscriptionsCreated = internalAction({
  ...
  handler: async (ctx): Promise<TestResult> => {
    ...
    try {
      // Create zones
      const zone1 = await ctx.runMutation(
        internal.testing.dispatchTests.createTestZone,
        { name: "Zone A" },
      );
      const zone2 = await ctx.runMutation(
        internal.testing.dispatchTests.createTestZone,
        { name: "Zone B" },
      );
      ...
      return {
        ...
        error: passed
          ? undefined
          : `Expected 4 subscriptions, got ${result.created}`,
      };
    } catch (e) {
      return {
        name: testName,
...
export const testDualDispatch = internalAction({
  ...
  handler: async (ctx): Promise<TestResult> => {
    ...
    try {
      ...
      const radiusInstructor = await ctx.runMutation(
        internal.testing.dispatchTests.createTestInstructor,
        {
      ...
      const zoneInstructor = await ctx.runMutation(
        internal.testing.dispatchTests.createTestInstructor,
        {
      ...
      const studioId = await ctx.runMutation(
        internal.testing.dispatchTests.createTestStudio,
        {
          name: "Test Studio",
          latitude: 32.0853,
          longitude: 34.7818,
      ...
      const jobId = await ctx.runMutation(
        internal.testing.dispatchTests.createTestJob,
        {
          studioId,
          title: "Test Job - HIIT",
          category: "hiit",
      ...
      const radiusMatches = await ctx.runQuery(
        internal.geo.findInstructorsForJobQuery,
        {
          jobPoint: { latitude: 32.0853, longitude: 34.7818 },
          jobCategory: "hiit",
      ...
      const zoneMatches = await ctx.runQuery(
        internal.zoneSubscriptions.findZoneInstructors,
        {
          zoneId,
          category: "hiit",
          requiresVerification: false,
      ...
      const foundRadius = radiusMatches.some(
        (m: { instructorId: Id<"users"> }) =>
          m.instructorId === radiusInstructor,
      );
      const foundZone = zoneMatches.some(
        (m: { instructorId: Id<"users"> }) => m.instructorId === zoneInstructor,
      );
      ...
      return {
        ...
        error: passed
          ? undefined
          : `Radius found: ${foundRadius}, Zone found: ${foundZone}`,
      };
    } catch (e) {
      return {
        name: testName,
    ...
    for (let i = 0; i < radiusModeCount; i++) {
      radiusPromises.push(
        ctx.runMutation(internal.testing.dispatchTests.createTestInstructor, {
          ...
          categories: ["yoga", "pilates"][Math.floor(Math.random() * 2)]
            ? ["yoga"]
            : ["pilates"],
        }),
      );
...
export const runAllTests = internalAction({
  ...
  handler: async (
    ...
    results.push(
      await ctx.runAction(
        internal.testing.dispatchTests.testRadiusModeGeoSync,
        {},
      ),
    );
    results.push(
      await ctx.runAction(
        internal.testing.dispatchTests.testZoneModeNotInGeo,
        {},
      ),
    );
    results.push(
      await ctx.runAction(
        internal.testing.dispatchTests.testZoneSubscriptionsCreated,
        {},
      ),
    );
    results.push(
      await ctx.runAction(internal.testing.dispatchTests.testDualDispatch, {}),
    );
